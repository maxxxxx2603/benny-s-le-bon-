...
// IDs des channels
const ANNOUNCE_CHANNEL_ID = '1210594716802883604';
const CV_REVIEW_CHANNEL_ID = '1458966535556038696';
const DISPO_CHANNEL_ID = '1458961018012172389';
const ROLE_ID = '1458961638316179648';
// Système de vente de kits / paie
const SALES_CHANNEL_ID = '1435628264608628736';
...
const CUSTOMS_CHANNEL_ID = '1435628264608628736'; // Channel pour les customisations
// Annonce recrutement via /rc
const RECRUIT_ANNOUNCE_CHANNEL_ID = '1459163785117040786';
const GUILD_ID = '1210592005076422736';
// Remplacez par l'ID du rôle Citoyen si vous le connaissez, sinon détection par nom
const CITIZEN_ROLE_ID = null;
const STAFF_ROLE_ID = '1363091238923931658';
const ID_CARD_CHANNEL_ID = '1427061446956679289';
const DIRECTION_ROLE_ID = '@everyone'; // Remplacez par l'ID du rôle direction si disponible
const COMMANDE_CATEGORY_ID = '1332876434259316859';
const CONTRAT_CATEGORY_ID = '1459164385829322949';
const TICKET_ANNOUNCE_CHANNEL_ID = '1387948740907892847'; // Channel pour l'annonce des tickets
...
const questions = [
    ...
    "Motivations pour rejoindre Benny's :",
    "Disponibilités :",
    "🪪 Pièce d'identité :"
];
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (interaction.commandName === 'add') {
            try {
                ...
                const welcomeEmbed = new EmbedBuilder()
                    .setTitle('👋 Bienvenue chez Benny\'s !')
                    .setDescription(`Félicitations ${targetUser}, vous êtes maintenant employé chez Benny's !\n\nVoici les commandes à votre disposition :`)
                    .addFields(
                        {
        ...
        if (interaction.commandName === 'info') {
            try {
                ...
                const mainEmbed = new EmbedBuilder()
                    .setTitle('🤖 Benny\'s BOT - Guide Complet')
                    .setDescription('Bienvenue sur le bot de gestion de Benny\'s ! Voici toutes les fonctionnalités disponibles.')
                    .setColor('#3498DB')
                    .setTimestamp();
        ...
        if (interaction.commandName === 'payes') {
            try {
                ...
                // Grouper les factures par employé
                const employeeData = {};

                // Calculer les totaux de factures (customs)
                for (const custom of customs.customs || []) {
                    if (!employeeData[custom.userId]) {
                        employeeData[custom.userId] = {
                            userTag: custom.userTag,
                            totalFactures: 0,
                            kits: 0
                        };
                    }
                    employeeData[custom.userId].totalFactures += custom.montant;
                }

                // Ajouter les données de kits
                for (const [userId, data] of Object.entries(payroll.users || {})) {
                    if (!employeeData[userId]) {
                        // Récupérer le membre pour obtenir son displayName
                        try {
                            const member = await interaction.guild.members.fetch(userId);
                            employeeData[userId] = {
                    ...
                    employeeData[userId].kits = data.kits || 0;
                }
                ...
                console.log('Nombre d\'employés dans employeeData:', Object.keys(employeeData).length);
                console.log('IDs:', Object.keys(employeeData));

                for (const [userId, data] of Object.entries(employeeData)) {
                    console.log(`\nTraitement de l'employé ${userId} (${data.userTag})`);
                    try {
                        const member = await interaction.guild.members.fetch(userId).catch(() => null);
                        
                        if (!member) {
                            console.log(`❌ Membre ${userId} non trouvé dans le serveur`);
                            // Afficher quand même avec les données disponibles
                            const quota = customs.quotas?.[userId] || 0;
                            const percentage = 15; // Par défaut ER
                            const payeFactures = Math.floor(data.totalFactures * (percentage / 100));
                            const primeKits = Math.floor(data.kits / 20) * 100000;
                            ...
                            embed.addFields({
                                name: `[?] ${data.userTag} (membre non trouvé)`,
                                value: `${statusQuota}\n**Factures:** ${fmt.format(data.totalFactures)}$ (${percentage}%) = ${fmt.format(payeFactures)}$\n**Kits:** ${data.kits} kits → Prime: ${fmt.format(primeKits)}$\n**💵 TOTAL: ${fmt.format(payeTotal)}$**${warning}`,
                                inline: false
                            });
                            hasEmployees = true;
                            continue;
